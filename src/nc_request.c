...
req_server_dequeue_imsgq(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    QMD_TAILQ_CHECK_NEXT(msg, s_tqe);/* tbd*/
    QMD_TAILQ_CHECK_PREV(msg, s_tqe);/* tbd*/
...
/** 
 * Modified to support duplicate write on multiple servers.
 * If write pool, select all servers in the selected group, get conn for each server.
 * Duplicate msg for each conn. 
 * 支持中文注释：
 * 通过nc之前的策略选中组后，在组内的所有服务器进行复写。
 * 方法：复制msg
 * Modified 2014.1.16 by Juncheng
 * Removed 2014.5.4 by Juncheng
 */
static void
req_forward(struct context *ctx, struct conn *c_conn, struct msg *msg)
{
    ...
    req_forward_stats(ctx, s_conn->owner, msg); /* only stat for one msg */
    loga("fffffffffforward from c %d to s %d req %"PRIu64" len %"PRIu32
