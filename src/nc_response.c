...
rsp_make_error(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    pmsg = msg->peer;
    if (pmsg != NULL) {
        /*ASSERT(!pmsg->request && pmsg->peer == msg); 2014.1.20*/
...
/*
 * changed to support write condition, herein one client msg has several server msg peers.
 * do nothing if not all the server msg peers are handled, count with msg->npeers_fin.
 * changed 2014.1.21
 * recovered 2014.1.24, change to msg->copy method.
 */
void
