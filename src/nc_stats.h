...
#define STATS_GROUP_CODEC(ACTION)                                                                            \
    /* group behavior */                                                                                   \
    ACTION( server_ejects,          STATS_COUNTER,      "# times backend server was ejected")               \
...
/**
 * add stats_group, like struct server_group. 20131225
 */
struct stats_group {
    struct string name;   /* group name (ref) */
    struct array  metric; /* stats_metric[] for group codec */
...
struct stats_pool {
    ...
    /*struct array  server;*/ /* stats_server[] */
    struct array  group; /* stats_group[] */
};
...
#undef DEFINE_ACTION
/* added 20131225*/
#define DEFINE_ACTION(_name, _type, _desc) STATS_GROUP_##_name,
typedef enum stats_group_field {
    STATS_GROUP_CODEC(DEFINE_ACTION)
    STATS_GROUP_NFIELD
} stats_group_field_t;
#undef DEFINE_ACTION
...
/* added 20131225 */
#define stats_group_incr(_ctx, _group, _name) do {                        \
    _stats_group_incr(_ctx, _group, STATS_GROUP_##_name);                  \
} while (0)

#define stats_group_decr(_ctx, _group, _name) do {                        \
    _stats_group_decr(_ctx, _group, STATS_GROUP_##_name);                  \
} while (0)

#define stats_group_incr_by(_ctx, _group, _name, _val) do {               \
    _stats_group_incr_by(_ctx, _group, STATS_GROUP_##_name, _val);         \
} while (0)

#define stats_group_decr_by(_ctx, _group, _name, _val) do {               \
    _stats_group_decr_by(_ctx, _group, STATS_GROUP_##_name, _val);         \
} while (0)
/* added end */
...
#define stats_group_incr(_ctx, _group, _name)

#define stats_group_decr(_ctx, _group, _name)

#define stats_group_incr_by(_ctx, _group, _name, _val)

#define stats_group_decr_by(_ctx, _group, _name, _val)
...
/* added 20131226 */
struct server_group;    /* Turn off the warning: "struct server_group’ declared inside parameter list" */

void _stats_group_incr(struct context *ctx, struct server_group *group, stats_group_field_t fidx);
void _stats_group_decr(struct context *ctx, struct server_group *group, stats_group_field_t fidx);
void _stats_group_incr_by(struct context *ctx, struct server_group *group, stats_group_field_t fidx, int64_t val);
void _stats_group_decr_by(struct context *ctx, struct server_group *group, stats_group_field_t fidx, int64_t val);
