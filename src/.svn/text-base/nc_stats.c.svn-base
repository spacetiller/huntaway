...
/* added 20131225 */
static struct stats_metric stats_group_codec[] = {
    STATS_POOL_CODEC( DEFINE_ACTION )
};
...
/* added 20131225 */
static struct stats_desc stats_group_desc[] = {
    STATS_POOL_CODEC( DEFINE_ACTION )
};
...
stats_metric_reset(struct array *stats_metric)
{
    ...
    /*loga("nmetric: %d, STATS_POOL_NFIELD: %d, STATS_GROUP_NFIELD: %d, STATS_SERVER_NFIELD: %d  \n",nmetric, STATS_POOL_NFIELD, STATS_GROUP_NFIELD, STATS_SERVER_NFIELD );*/
    ASSERT(nmetric == STATS_POOL_NFIELD || nmetric == STATS_GROUP_NFIELD || nmetric == STATS_SERVER_NFIELD);
...
/**
 * add stats_group_metric_init()
 * added by Juncheng,20131225
 *
 */
static rstatus_t
stats_group_metric_init(struct array *stats_metric)
{
    rstatus_t status;
    uint32_t i, nfield = STATS_GROUP_NFIELD;
    ...
    for (i = 0; i < nfield; i++) {
        ...
        /* initialize from group codec first */
        *stm = stats_group_codec[i];
...
/**
 * add stats_group_init, like stats_pool_init, between server and pool 
 * added by Juncheng, 20131225
 *
 */
static rstatus_t
stats_group_init(struct stats_group *stg, struct server_group *sg)
{
    rstatus_t status;

    stg->name = sg->name;
    array_null(&stg->metric);
    array_null(&stg->server);

    status = stats_group_metric_init(&stg->metric);
    if (status != NC_OK) {
        return status;
    }
    
    status = stats_server_map(&stg->server, &sg->server);
    if (status != NC_OK) {
        return status;
    }

    log_debug(LOG_VVVERB, "init stats pool '%.*s' with %"PRIu32" metric and "
              "%"PRIu32" server", stg->name.len, stg->name.data,
              array_n(&stg->metric), array_n(&stg->metric));

    return NC_OK;
}

/**
 * add stats_group_map, to map pool-group-server to stats counterpart
 * added by Juncheng, 20131225
 *
 */
static rstatus_t
stats_group_map(struct array *stats_group, struct array *group)
{
    rstatus_t status;
    uint32_t i, ngroup;

    ngroup = array_n(group);
    ASSERT(ngroup != 0);

    status = array_init(stats_group, ngroup, sizeof(struct stats_group));
    if (status != NC_OK) {
        return status;
    }

    for (i = 0; i < ngroup; i++) {
        struct server_group *g = array_get(group, i);
        struct stats_group *stg = array_push(stats_group);

        status = stats_group_init(stg, g);
        if (status != NC_OK) {
            return status;
        }
    }

    log_debug(LOG_VVVERB, "map %"PRIu32" stats groups", ngroup);

    return NC_OK;
}

/**
 * add stats_group_ummap, to map pool-group-server to stats counterpart
 * added by Juncheng, 20131225
 *
 */
static void
stats_group_unmap(struct array *stats_group)
{
    uint32_t i, ngroup;

    ngroup = array_n(stats_group);

    for (i = 0; i < ngroup; i++) {
        struct stats_group *stg = array_pop(stats_group);
        stats_metric_deinit(&stg->metric);
        stats_server_unmap(&stg->server);
    }
    array_deinit(stats_group);

    log_debug(LOG_VVVERB, "unmap %"PRIu32" stats groups", ngroup);
}
...
stats_pool_init(struct stats_pool *stp, struct server_pool *sp)
{
    ...
    status = stats_group_map(&stp->group, &sp->group);
    if (status != NC_OK) {
...
/**
 * modified to add group stuff, 20131227
 */
static void
stats_pool_reset(struct array *stats_pool)
{
    ...
    for (i = 0; i < npool; i++) {
        ...
        ngroup = array_n(&stp->group);
        for (j = 0; j < ngroup; j++) {
            struct stats_group *stg = array_get(&stp->group, i);
            uint32_t k, nserver;
    
            stats_metric_reset(&stg->metric);
    
            nserver = array_n(&stg->server);
            for (k = 0; k < nserver; k++) {
                struct stats_server *sts = array_get(&stg->server, k);
...
stats_create_buf(struct stats *st)
{
    ...
    uint32_t group_extra = 8;       /* '"group_name": { ' + ' }' */
    ...
    for (i = 0; i < array_n(&st->sum); i++) {
        ...
        /* groups per pool */
        for (j = 0; j < array_n(&stp->group); j++) {
            struct stats_group *stg = array_get(&stp->group, j);
            uint32_t h;
    
            size += stg->name.len;
            size += group_extra;
    
            for (h = 0; h < array_n(&stg->metric); h++) {
                struct stats_metric *stm = array_get(&stp->metric, h);
            ...
            /* servers per group */
            for (h = 0; h < array_n(&stg->server); h++) {
                struct stats_server *sts = array_get(&stg->server, h);
                uint32_t k;
...
stats_aggregate(struct stats *st)
{
    ...
    for (i = 0; i < array_n(&st->shadow); i++) {
        ...
        for (j = 0; j < array_n(&stp1->group); j++) {
            struct stats_group *stg1, *stg2;
            uint32_t k;

            stg1 = array_get(&stp1->group, j);
            stg2 = array_get(&stp2->group, j);
            stats_aggregate_metric(&stg2->metric, &stg1->metric);

            for (k = 0; k < array_n(&stg1->server); k++) {
                struct stats_server *sts1, *sts2;

                sts1 = array_get(&stg1->server, k);
                sts2 = array_get(&stg2->server, k);
...
stats_make_rsp(struct stats *st)
{
    ...
    for (i = 0; i < array_n(&st->sum); i++) {
        ...
        for (j = 0; j < array_n(&stp->group); j++) {
            ...
            status = stats_begin_nesting(st, &stg->name);
            if (status != NC_OK) {
                return status;
            }

            /* copy group metric from sum(c) to buffer */
            status = stats_copy_metric(st, &stg->metric);
            if (status != NC_OK) {
                return status;
            }

            for (k = 0; k < array_n(&stg->server); k++) {
                struct stats_server *sts = array_get(&stg->server, k);
...
/**
 * add group stats functions
 * added 20131225
 *
 */
static struct stats_metric *
stats_group_to_metric(struct context *ctx, struct server_group *group,
                     stats_group_field_t fidx)
{
    struct stats *st;
    struct stats_group *stg;
    struct stats_metric *stm;
    uint32_t pidx;
    ...
    st = ctx->stats;
    stg = array_get(&st->current, pidx);
    stm = array_get(&stg->metric, fidx);

    st->updated = 1;

    log_debug(LOG_VVVERB, "metric '%.*s' in group %"PRIu32"", stm->name.len,
              stm->name.data, pidx);
...
void
_stats_group_incr(struct context *ctx, struct server_group *group,
                 stats_group_field_t fidx)
{
    struct stats_metric *stm;

    stm = stats_group_to_metric(ctx, group, fidx);
...
void
_stats_group_decr(struct context *ctx, struct server_group *group,
                 stats_group_field_t fidx)
{
    struct stats_metric *stm;
...
void
_stats_group_incr_by(struct context *ctx, struct server_group *group,
                    stats_group_field_t fidx, int64_t val)
{
    struct stats_metric *stm;
...
void
_stats_group_decr_by(struct context *ctx, struct server_group *group,
                    stats_group_field_t fidx, int64_t val)
{
    struct stats_metric *stm;
                       ...
                       stats_server_field_t fidx)
{
    ...
    struct stats_metric *stm;
    uint32_t pidx, gidx, sidx;

    sidx = server->idx;
    gidx = server->owner->idx;
    ASSERT(server->owner!=NULL && server->owner->owner!=NULL);
    pidx = server->owner->owner->idx; 

    st = ctx->stats;
    stp = array_get(&st->current, pidx);
    stg = array_get(&stp->group, gidx);
    sts = array_get(&stg->server, sidx);
