...
/* add conf constant, Juncheng,20131224*/
/* Note: constant here not proper because there must be counterpart in nc_server.h */
/*#define CONF_RW_READ   0
#define CONF_RW_WRITE  1
#define CONF_RW_ALL    -1*/  /* compatible to original conf */
...
struct conf_server {
    ...
    unsigned        valid:1;    /* valid? */
    uint8_t         role;       /* redis server role: m-master n-secondary master s-slave  */
    int             gid;        /* group label, juncheng 20131205 */
};

struct conf_group{
    struct string   name;       /* group: as "name:port:weight|group:role" */
    struct array    server;     /* servers: conf_server[] */
    int             weight;     /* weight */
    int             gid;        /* group label, juncheng 20131205 */
    /*int             write;  */    /* group role, empty  */
};
...
struct conf_pool {
    ...
    /*struct array       server;*/            /* servers: conf_server[] */
    struct array       group;                 /* server groups: conf_group[] juncheng 20131217 */
    /*int                write; */                /* pool role: 1-write 0-read 2-all r/w, compatible downwards  */
    unsigned           valid:1;               /* valid? */
};
...
struct conf_group *check_group_each(void *elem, void *data);
rstatus_t conf_server_each_transform(void *elem, void *data);
rstatus_t conf_group_each_transform(void *elem, void *data);/*added*/
